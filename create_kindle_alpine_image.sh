...
apk add xorg-server-xephyr xwininfo xdotool xinput dbus-x11 sudo bash nano git vim
apk add desktop-file-utils gtk-engines consolekit gtk-murrine-engine caja caja-extensions marco gnome-themes-extra gnome-chess
...
# CREATING IMAGE FILE
# To create the image file, a file full of zeros with the desired size is created using dd. An ext4-filesystem is created in it.
...
# MOUNTING IMAGE
# The mountpoint is created (doesn't matter if it exists already) and the empty ext4-filsystem is mounted in it
echo "Mounting image"
mkdir -p "$MNT"
